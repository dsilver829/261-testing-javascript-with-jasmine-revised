describe "CreditCard", ->
  it "strips out spaces and dashes from the number", ->
    card = new CreditCard("1 2 -3")
    expect(card.number).toBe("123")

  it "validates the number using mod 10", ->
  ...
  it "validates number when the field loses focus", ->
